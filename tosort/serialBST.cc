...
public:
    ...
    TreeNode* buildTree2(){
        if(vi.empty()) { return NULL;}
        return preOrder0(INT_MIN, INT_MAX);
    }
    ...
    TreeNode* preOrder3(int p, int gp){
        TreeNode* node = NULL;
        if((i < vi.size()) && ((vi[i] < p) || (p < vi[i] && vi[i] < gp))) {
            node = new TreeNode(vi[i]);
            i++;
            node->left = preOrder0(node->val, p);            
            node->right = preOrder0(node->val, p);
    ...
    TreeNode* preOrder0(int mini, int maxi){
        TreeNode* node = NULL;
        if((i < vi.size()) && (vi[i] > mini) && (vi[i] < maxi)) {
            node = new TreeNode(vi[i]);
            i++;
            node->left = preOrder0(mini, node->val);            
            node->right = preOrder0(node->val, maxi);
        }        
        return node;
    }    
